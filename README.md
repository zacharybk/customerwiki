...
- The repo includes a `netlify.toml` with the recommended Next.js plugin. Ensure Netlify installs devDependencies (default) so `@netlify/plugin-nextjs` runs during the build.
- Build settings:
  - **Base directory:** `/`
  - **Build command:** `npm run build`
  - **Publish directory:** `.next`
  - **Functions directory:** `netlify/functions` (auto-generated by the plugin)
  - **Node version:** 18+ (set via `NODE_VERSION` in `netlify.toml`)
